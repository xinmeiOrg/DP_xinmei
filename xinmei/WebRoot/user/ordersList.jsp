		    ...
		    success: function(data) {
		        if(data.status==1){
		        	...
		        	for(var i=0;i<list.length;i++){
		        		...
		        		$("#orderList").append("<div class='order-list' id='"+list[i].id+"'><a href='ordersDetail?no="+list[i].no+"'><div class='cart-title wbox'><div class='of f30'>订单号&nbsp;&nbsp;"+list[i].no.substring(0,10).concat('...')+"</div><div class='wbox-flex tr sn-txt-muted'>应付款 <i class='f30 sn-txt-black'> ￥"+list[i].money+" </i></div></div></a><ul class='order-pro-list order-box'><li id='"+list[i].id+"_'><a href='ordersDetail?no="+list[i].no+"' class='pro-list'><div class='wbox'><div class='pro-info wbox-flex'><div class='pro-name text-clamp2 fs26'>"+list[i].productName+"</div><div class='snPrice sn-txt-black fs30'>"+date+"</div><div class='list-opra sn-txt-muted'>"+status+"&nbsp;&nbsp;&nbsp;&nbsp;数量："+list[i].productNum+"</div></div></div></a></li></ul></div>");
		        	}
